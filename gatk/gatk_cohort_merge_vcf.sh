...
while read TARGGENE CHR LOCUS_BEGIN LOCUS_END; do
	cd /rstore/user/$USER/default/VCFs/GenesOfInterest/$TARGGENE 	
...
# get list of cohorts
	...
	SUFFIX="vcfBeta-ALL-ASM.$TARGGENE.vcf"
	mkdir -p /rstore/user/$USER/default/VCFs/GenesOfInterest/$TARGGENE/MERGED /rstore/user/$USER/default/VCFs/GenesOfInterest/$TARGGENE/JOBS

# for each group
	for GROUP in $GROUP_LIST; do 
		GROUP_OUT="/rstore/user/$USER/default/VCFs/GenesOfInterest/$TARGGENE/MERGED/$GROUP.$SUFFIX"

    # get list of samples
	    	SAMPLES=`ls /rstore/user/$USER/default/VCFs/GenesOfInterest/$TARGGENE/$GROUP.vcfBeta-GS*.vcf \
/rstore/user/$USER/default/VCFs/GenesOfInterest/$TARGGENE/$GROUP.vcfBeta-NA*.vcf 2>/dev/null`
...
# build variant list
	...
	for S in $GROUP_LIST; do 
		INDEX=$(($INDEX+1))
		PRIO=v$INDEX
		VAR_LIST="$VAR_LIST --variant:$PRIO /rstore/user/$USER/default/VCFs/GenesOfInterest/$TARGGENE/MERGED/$S.$SUFFIX"
		if [ -n "$PRIO_LIST" ]; then PRIO_LIST="$PRIO_LIST,$PRIO"; else PRIO_LIST=$PRIO; fi
	done
	ALL_GROUP_OUT=/rstore/user/$USER/default/VCFs/GenesOfInterest/$TARGGENE/MERGED/$ALL_GROUP.$SUFFIX
